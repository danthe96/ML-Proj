...
def train(data, num_epochs=30):
    data = np.zeros((500, 3))  # TODO (T, d)

    # First, we build the model and get pointers to the model parameters
    x, cost, W, bh, bv, lr, Wuh, Wuv, Wvu, Wuu, bu, u0 = rnn_rbm.rnnrbm(input_x.shape[1])
...
def predict(data, saved_weights_path, min_energy):
    # This function takes as input the path to the weights of the network
    x, cost, W, bh, bv, lr, Wuh, Wuv, Wvu, Wuu, bu, u0, ebm_layer_shape = rnn_rbm.rnnrbm()
