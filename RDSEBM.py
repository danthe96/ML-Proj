...
class RDSEBM(object):
  ...
  """ Recurrent Deep-Structured Energy Based Model
  ...
  def _build_model(self):
    ...
    for i in range(len(self.layer_shape) - 2, -1, -1):
      result = tf.sigmoid(tf.matmul(self.W[i + 1], h[i + 1]) + self.bias[i]) * tf.matmul(self.W[i + 1], result)
  ...
  def _create_variables(self):
    ...
    for i in range(0, len(self.layer_shape)):
      W[i] = tf.Variable(tf.random_normal((self.layer_shape[i - 1], self.layer_shape[i]), mean=0.0, stddev=0.01),
                         name='weights' + str(i))
